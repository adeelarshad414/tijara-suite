...
.PHONY: up down logs shell restart ps validate js-check security-audit config install-suite seed-pos-demo seed-e2e test-odoo e2e e2e-staging ops-staging release-candidate signoff-pack bridge-up bridge-logs bridge-ps backup-db restore-drill provision-tenant provision-tenant-ops hardware-cert-smoke load-smoke container-scan dependency-scan monitoring-up monitoring-logs monitoring-drill
