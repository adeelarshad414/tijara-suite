...
.PHONY: up down logs shell restart ps validate js-check security-audit config install-suite seed-pos-demo seed-e2e test-odoo e2e e2e-staging ops-staging operations-release-bundle release-candidate signoff-pack check-release-readiness staging-release-signoff production-deployment-gate production-rollback production-smoke certification-evidence psp-readiness-evidence psp-fixture-smoke fbr-readiness-evidence monitoring-evidence incident-runbook-evidence load-evidence load-profile load-enterprise-surfaces load-profile-matrix-evidence bridge-up bridge-logs bridge-ps backup-db restore-drill provision-tenant provision-tenant-ops hardware-cert-smoke load-smoke container-scan dependency-scan monitoring-up monitoring-logs monitoring-drill
