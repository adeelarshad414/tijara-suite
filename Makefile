...
py-start:
	python3 scripts/tijara_services.py start

py-stop:
	python3 scripts/tijara_services.py stop

py-restart:
	python3 scripts/tijara_services.py restart

py-status:
	python3 scripts/tijara_services.py status

py-config:
	python3 scripts/tijara_services.py config
...
tijara-cloud-domain-deploy:
	bash scripts/tijara-cloud-domain-deploy.sh $(TIJARA_CLOUD_DOMAIN_FLAGS)
...
upgrade-pkr-gst:
	$(COMPOSE) run --rm odoo bash /usr/local/bin/tijara-start-odoo -d $(DB) -u tijara_base,tijara_retail_core,tijara_pos_pk,tijara_pos_experience,tijara_analytics,tijara_ecommerce,tijara_demo_pos --without-demo --stop-after-init
...
verify-enterprise-seed:
	$(COMPOSE) exec -T odoo odoo shell --config="$${ODOO_RUNTIME_CONFIG:-/tmp/tijara-odoo.conf}" -d $(DB) < scripts/verify_enterprise_seed.py

seed-e2e:
	bash scripts/seed_e2e_odoo.sh

local-e2e-evidence:
	bash scripts/run_local_browser_e2e_evidence.sh
...
browser-e2e-matrix:
	bash scripts/run_browser_e2e_matrix.sh

protected-browser-e2e:
	bash scripts/run_protected_browser_e2e_evidence.sh

protected-browser-e2e-matrix:
	TIJARA_TARGET_ENVIRONMENT=$${TIJARA_TARGET_ENVIRONMENT:-staging} bash scripts/run_browser_e2e_matrix.sh

protected-pos-matrix-evidence:
	python3 scripts/export_protected_pos_matrix_evidence.py $(TIJARA_PROTECTED_POS_MATRIX_FLAGS)
...
infra-provider-readiness:
	python3 scripts/export_infra_provider_readiness.py $(TIJARA_INFRA_PROVIDER_READINESS_FLAGS)

protected-release-chain:
	python3 scripts/run_protected_release_evidence_chain.py $(TIJARA_PROTECTED_CHAIN_FLAGS)
...
protected-offline-queue-snapshot:
	python3 scripts/export_protected_offline_queue_snapshot.py

protected-offline-pilot-evidence:
	python3 scripts/export_protected_offline_pilot_evidence.py
...
assumed-certification-evidence:
	python3 scripts/export_assumed_external_certification_evidence.py
...
monitoring-dashboards-check:
	python3 scripts/generate_grafana_dashboards.py --check
